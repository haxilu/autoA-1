...
public class GBData {
    ...
    ////多点模糊找色
    public static int[] MultiPointFindColor(int mainColross, String subColors, double distance, int x1, int y1, int x2, int y2){
        assert bitmap != null;
        ArrayList<Integer[]> subcs= GBData.getColorListSub(subColors);//16进制文本颜色转成10进制颜色数组（子颜色列表）
        int[] mainColors = color16To10_int(mainColross);//文本转10进制颜色（主颜色）
        for (int i = x1; i < x2; i++) {
            for (int j =  y1; j < y2; j++) {
                double _distance = SinglePointColorContrast_LAB(getRGB(bitmap,i,j),mainColors);//当前颜色和主颜色对比
                if (_distance<=distance){    // <= 欧氏距离 = 找到颜色
